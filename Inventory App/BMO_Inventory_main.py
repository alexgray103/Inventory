...
import smtplib, ssl
from email_attach import email_sender
...
class inventory_main:
    def __init__(self, parent_frame):
        ...
        self.csv_location = '/home/pi/Desktop/Inventory-main/Inventory_App/LAB_BOM.csv'
       ...
       # Add that New frame To a Window In The Canvas
        ...
        new_part.grid(row =1, column = 2, padx = 10, pady = 4, sticky = 'nsew')
        
        self.viewer = Button(self.second_frame, text = "Sort by vendor",
                           command = self.view_change, height = 7)
        self.viewer.grid(row =1, column = 1, padx = 10, pady = 4, sticky = 'nsew')
        self.check_page()
        
    def view_change(self):
        
        ## erase all the widgets for eahc inventory frame
        for widget in self.type_frame:
            widget.grid_forget()
            
        if self.view_handler ==9:
            self.view_handler =2
            self.viewer.configure(text = 'Sort by part type')
        else:
            self.view_handler =9
            self.viewer.configure(text = 'Sort by vendor')
    ...
    def send_email(self):
        ...
        with smtplib.SMTP_SSL("smtp.gmail.com", port, context = context) as server:
            server.login("alex_gray@my.uri.edu", "wormwood")
    ...
    def create_bom(self):
        ...
        BOM_path = '/home/pi/Desktop/Inventory-main/Inventory_App/'
        needed = list(range(self.row))
        cost = list(range(self.row))
        # get the important information
        new_df = self.df.iloc[self.bom_values, [0,1,2,7]]
        
        c = 0
        r = 1
        last_row = 23
        
        for info in self.bom_values:
            RQ = self.df.iloc[info, 3]
            information = self.df.iloc[info, 8]
            needed[info] = int(2*RQ - information)
            cost[info] = float(self.df.iloc[info, 6].replace('$',''))*needed[info]
            low = '-' + str(self.df.iloc[info,0]) + ' (qty: ' + str(needed[info]) + ')'
        ...
        columns_titles = ["Part", "Description", "Vendor", "Qty", "Cost", "link"]
        new_df=new_df.reindex(columns=columns_titles)
        self.save_file = '/home/pi/Desktop/Inventory-main/Inventory_App/bom.csv'
        #self.save_file = '/home/pi/Documents/Inventory-main/Inventory_App/bom.csv'
        new_df.to_csv(self.save_file, mode = 'w', index=False)
        
        self.emailer.send_email('rjadeja@bu.edu','/home/pi/Desktop/Inventory-main/Inventory_App/bom.csv')
        self.emailer.send_email('algray@bu.edu','/home/pi/Desktop/Inventory-main/Inventory_App/bom.csv')
        self.emailer.send_email('bmolab512@gmail.com','/home/pi/Desktop/Inventory-main/Inventory_App/bom.csv')
    ...
    def inventory_update(self):
        ...
        self.type_label = list(range(250))
        ...
        for loc in range(self.row):
            ...
            self.type =  inventory_row[self.view_handler]
        ...
        for x in range(self.row):
            ...
            if build_type:
                ...
                for item_type in self.type_list:
                    ...
                    self.type_frame[item].grid(row = item+2, column = 0, columnspan = 3, padx = padding, pady = 5)
                    
                    self.type_label[item] = Label(self.type_frame[item], font = self.label_font,
                                       bg = 'gray80', text = item_type)
                    self.type_label[item].grid(row = 0, column = 1,  sticky = 'nsew')
                    item+=1
            ...
            #check how many items are in this row to move our label around
            #if self.col_handler[item_type_handler] < 3 and self.row_handler[item_type_handler] ==1:
    ...
    def check_page(self):
        ...
        for x in range(self.row):
            ...
            if  int(inventory_row[8]) < int(2*int(inventory_row[3])) and int(inventory_row[8]) >= int(inventory_row[3]):
                ...
                self._inventory_needed = self.return_needed_val(self.inventory_needed, x)
        self.bom_values = self.inventory_needed + self.inventory_low
